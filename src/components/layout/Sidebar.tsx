...
export default function Sidebar({ 
  ...
  const handleRemoveFloor = () => {
    setFloorCount(prev => Math.max(0, prev - 1));
  };
  ...
  const handleOffsetUp = () => {
    setVerticalOffset(prev => prev + 50);
  };

  const handleOffsetDown = () => {
    setVerticalOffset(prev => prev - 50);
  };
  ...
  return (
    <div className="w-1/2 bg-white p-8 relative flex flex-col h-full">
      ...
      <div className="absolute left-4 bottom-60 bg-white rounded-full shadow-lg p-2 flex flex-col gap-2">
        <button 
          onClick={handleOffsetUp} 
          className="p-2 hover:bg-gray-100 rounded-full"
        >
          <ChevronUp size={20} />
        </button>
        <button 
          onClick={handleOffsetDown} 
          className="p-2 hover:bg-gray-100 rounded-full"
        >
          <ChevronDown size={20} />
        </button>
